...
async function start() {
  ...
  try {
    testSuitePaths = await getTestSuitePaths();
  } catch (err) {
    console.error("An unexpected error occurred when getting Test Suites.\nTest run cancelled.")
    console.error(err);
    process.exit(1);
  }
  ...
  try {
    ...
    console.error("An unexpected error occurred when running Test Suites.\nTest run cancelled.")
    console.error(err);
    process.exit(1);
  }
  ...
  console.log(
    `Total run time: ${totalDuration / 1000}s, executed in: ${
      timeTaken / 1000
    }s, saved ${timeSaved / 1000} (~${Math.round(
